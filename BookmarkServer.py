...
import http.server
import requests
import threading  # to handle multiple requests
from urllib.parse import unquote, parse_qs
from socketserver import ThreadingMixIn  # functionality for mult. requests
...
def CheckURI(uri, timeout=5):
    ...
    '''
    try:
        return requests.get(uri, timeout=timeout).status_code == 200
...
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8000))  # Use PORT if it's there (heroku)
    server_address = ('', port)
